  ...
  return {
    ...
    async transform(from, code) {
      ...
      traverse(parsed, {
        ...
        'ExportDeclaration'({node, scope}) {
          if (isEntry) {
            for (const s of node.specifiers) {
              if (s.exported.name === '__update') {
                updateUid = scope.generateUid(s.local.name)
            ...
            if (node.declaration) {
              for (const d of node.declaration.declarations) {
                if (d.id.name === '__update') {
                  updateUid = scope.generateUid('__update')
            ...
            return {
              visitor: {
                ...
                'VariableDeclarator'({node}) {
                  if (updateUid != null && node.id.name === updateUid) {
                    const container = []
                ...
                'CallExpression'(path) {
                  ...
                  if (updateUid && path.node.callee.name === updateUid) {
                    ...
                    path.replaceWith(
                      t.callExpression(t.identifier(runUid), [path.node.callee])
                    )
                  }
                },
                'ImportDeclaration|ExportNamedDeclaration|ExportAllDeclaration'(
