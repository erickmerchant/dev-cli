...
export const createEntryPlugin = (map, isDev) => () => {
  let mainUid
  let runUid
  let useUid
  ...
  return {
    visitor: {
              ...
              path.pushContainer(
                ...
                t.callExpression(t.identifier(useUid), [
                  t.newExpression(t.identifier('URL'), [
                    ...
                    t.templateLiteral(
                      [
                        t.templateElement({raw: 'http://', cooked: 'http://'}),
      ...
      VariableDeclarator({node}) {
        if (isDev && mainUid != null && node.id.name === mainUid) {
          const container = []
          ...
          node.init.params.unshift(t.objectPattern(container))
        }
      },
